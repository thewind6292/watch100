...
if(! function_exists('pandora_woo_testimonial_widget'))
{
	function pandora_woo_testimonial_widget() {
		register_widget('Pandora_WooTestimonial_Widget');
	}
}
add_action('widgets_init', 'pandora_woo_testimonial_widget');


// get testimonial carousel theme
if(! function_exists('pandora_woo_testimonial_themes'))
{
	function pandora_woo_testimonial_themes($active, $type = "list") {
		$themes = ($type == "list") ? "" : array();
		$path 	= get_template_directory() . '/vc-templates/testimonial';
...
// get option testimonial carousel theme
if(! function_exists('pandora_option_testimonial_carousel_themes'))
{
	function pandora_option_testimonial_carousel_themes(){
		$pandora_woo_testimonial_themes        = pandora_woo_testimonial_themes("","array");
		$pandora_option_testimonial_carousel_themes = array();
		foreach ($pandora_woo_testimonial_themes as $key => $value) {
			$pandora_option_testimonial_carousel_themes_item    = str_replace(".php","",$value);
			$pandora_option_testimonial_carousel_themes_image   = str_replace(".php",".jpg",$value);
			$pandora_option_testimonial_carousel_themes[$pandora_option_testimonial_carousel_themes_item] = get_template_directory_uri() . '/vc-templates/testimonial/'.$pandora_option_testimonial_carousel_themes_image;
		}

		return $pandora_option_testimonial_carousel_themes;
	}
}
...
{
	class Pandora_WooTestimonial_Widget extends WP_Widget 
	{	
		/**
		...
		{
			parent::__construct(
				'pandora_woo_testimonial', // Base ID
				esc_html__('Pandora Testimonial', 'pandora'), // Name
		...
		public function widget($args, $instance) 
		{
			...
			if($total_items && is_file(get_template_directory() . '/vc-templates/testimonial/' . $carousel_theme)) {
				include get_template_directory() . '/vc-templates/testimonial/' . $carousel_theme;
			}
		...
		public function form($instance) 
		{
			...
			$unique_id 		= !empty($instance['unique_id']) ? $instance['unique_id'] : "pandora-woo-testimonial-".$random;
			...
			<p>
				...
				<em><?php _e('Enter unique ID for this carousel. Eg: pandora-woo-testimonial, pandora-woo-testimonial-1, pandora-woo-testimonial-2 ...', 'pandora'); ?></em>
			</p>
			<p>
			...
			<p>
				...
				<select class="widefat" name="<?php echo esc_attr($this->get_field_name('carousel_theme')); ?>" id="<?php echo esc_attr($this->get_field_id('carousel_theme')); ?>">				
					<?php echo pandora_woo_testimonial_themes($carousel_theme); ?>
				</select>
			</p>
			<p>
		...
		public function update($new_instance, $old_instance) 
		{
			...
			$instance['unique_id'] 		= (!empty($new_instance['unique_id'])) ? strip_tags($new_instance['unique_id']) : 'pandora-woo-testimonial';
		 ...
		 * [pandora_woo_testimonial class_suffix="WIDGET_CLASS_SUFFIX" ...]
		 */
		 
		public static function shortcode($atts, $content = "")
		{
			$widget = new Pandora_WooTestimonial_Widget;
...
if(! class_exists('PandoraWooTestimonialAddonClass')) 
{
	class PandoraWooTestimonialAddonClass 
	{
		...
		public function integrateWithVC() {
			...
			*/
			...
			vc_map(array(
				"name" 			=> esc_html__("Pandora Testimonial", 'pandora'),
				"description" 	=> esc_html__("A responsive carousel for WordPress.", 'pandora'),
				"base" 			=> "pandora_woo_testimonial",
				"class" 		=> "",
				"controls" 		=> "full",
				"icon" 			=> "icon-wpb-wp",
				...
				"params" => array(
					array(
						...
						"value" 		=> "pandora-woo-testimonial-".$random,
						"admin_label" 	=> true,
						"description" 	=> esc_html__('Enter unique ID for this carousel. Eg: pandora-woo-testimonial, pandora-woo-testimonial-1, pandora-woo-testimonial-2 ...', 'pandora'),
					),
					array(
						"type" 			=> "textfield",
						"save_always" 	=> true,
					...
					array(
						...
						'admin_label' => true,
						'options'     => pandora_option_testimonial_carousel_themes(),
					),
					array(
						"type" 			=> "dropdown",
						"save_always" 	=> true,
...
new PandoraWooTestimonialAddonClass();