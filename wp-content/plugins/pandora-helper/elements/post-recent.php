...
// registered Post Recent widget
if(! function_exists('pandora_post_recent_widget'))
{
	function pandora_post_recent_widget() {
		register_widget('Pandora_PostRecent_Widget');
	}
}
add_action('widgets_init', 'pandora_post_recent_widget');


// get Post Recent theme
if(! function_exists('pandora_post_recent_themes'))
{
	function pandora_post_recent_themes($active, $type = "list") {
		$themes = ($type == "list") ? "" : array();
		$path 	= get_template_directory() . '/vc-templates/post-recent';
...
if(! function_exists('pandora_option_post_recent_themes'))
{
	function pandora_option_post_recent_themes(){
		$pandora_post_recent_themes        = pandora_post_recent_themes("","array");
		$pandora_option_post_recent_themes = array();
		foreach ($pandora_post_recent_themes as $key => $value) {
			$pandora_option_post_recent_themes_item    = str_replace(".php","",$value);
			$pandora_option_post_recent_themes_image   = str_replace(".php",".jpg",$value);
			$pandora_option_post_recent_themes[$pandora_option_post_recent_themes_item] = get_template_directory_uri() . '/vc-templates/post-recent/'.$pandora_option_post_recent_themes_image;
		}
		return $pandora_option_post_recent_themes;
	}
}
...
// Pandora Post Recent Widget Class
if(! class_exists('Pandora_PostRecent_Widget')) 
{
	class Pandora_PostRecent_Widget extends WP_Widget 
	{	
		/**
		...
		{
			parent::__construct(
				'pandora_post_recent', // Base ID
				esc_html__('Pandora Post Recent', 'pandora'), // Name
				array('description' => esc_html__('A widget that display Wordpress Posts Recent in responsive.', 'pandora')) // Args
			);
		}
		...
		public function widget($args, $instance) 
		{
			...
			if($total_items && is_file(get_template_directory() . '/vc-templates/post-recent/' . $carousel_theme)) {
				include get_template_directory() . '/vc-templates/post-recent/' . $carousel_theme;
			}
		...
		public function form($instance) 
		{
			...
			$unique_id 		= !empty($instance['unique_id']) ? $instance['unique_id'] : "pandora-post-recent-".$random;
			...
			<p>
				...
				<em><?php _e('Enter unique ID for this carousel. Eg: pandora-post-recent, pandora-post-recent-1, pandora-post-recent-2 ...', 'pandora'); ?></em>
			</p>
			<p>
			...
			<p>
				...
				<select class="widefat" name="<?php echo esc_attr($this->get_field_name('carousel_theme')); ?>" id="<?php echo esc_attr($this->get_field_id('carousel_theme')); ?>">				
					<?php echo pandora_post_recent_themes($carousel_theme); ?>
				</select>
			</p>
			<p>
		...
		public function update($new_instance, $old_instance) 
		{
			...
			$instance['unique_id'] 		= !empty($new_instance['unique_id']) ? strip_tags($new_instance['unique_id']) : 'pandora-post-recent';
		 ...
		 * [Pandora_post_recent class_suffix="WIDGET_CLASS_SUFFIX" ...]
		 */
		 
		public static function shortcode($atts, $content = "")
		{
			$widget = new Pandora_PostRecent_Widget;
...
if(! class_exists('PandoraPostRecentAddonClass')) 
{
	class PandoraPostRecentAddonClass 
	{
		...
		public function integrateWithVC() {
			...
			*/
			...
			vc_map(array(
				"name" 			=> esc_html__("Pandora Post Recent", 'pandora'),
				"description" 	=> esc_html__("A responsive recent post for WordPress.", 'pandora'),
				"base" 			=> "pandora_post_recent",
				"class" 		=> "",
				"controls" 		=> "full",
				"icon" 			=> "icon-wpb-wp",
				...
				"params" => array(
					array(
						...
						"value" 		=> "pandora-post-recent-".$random,
						"admin_label" 	=> true,
						"description" 	=> esc_html__('Enter unique ID for this carousel. Eg: pandora-post-recent, pandora-post-recent-1, pandora-post-recent-2 ...', 'pandora'),
					),
					array(
						"type" 			=> "textfield",
						"save_always" 	=> true,
					...
					array(
						...
						'admin_label' => true,
						'options'     => pandora_option_post_recent_themes(),
					),
					array(
						"type" 			=> "textfield",
						"save_always" 	=> true,
...
new PandoraPostRecentAddonClass();