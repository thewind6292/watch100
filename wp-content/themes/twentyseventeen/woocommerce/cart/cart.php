		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
				...
				<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					<td class="">
						<?php
							...
							if ( ! $product_permalink ) {
								...
								printf( '<a class="hidden-xs hidden-sm" href="%s">%s</a>', esc_url( $product_permalink ), $thumbnail );
							}
						?>
					...
					<TD data-title="Giá :"><p><strong><?php
							echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key );
						?></strong></p></TD>

					<td class="product-quantity" data-title="<?php _e( 'Số lượng', 'woocommerce' ); ?>">
						<?php
					...
					<td class="product-subtotal" data-title="<?php _e( 'Tổng', 'woocommerce' ); ?>">
						<?php
							echo apply_filters( 'woocommerce_cart_item_subtotal', WC()->cart->get_product_subtotal( $_product, $cart_item['quantity'] ), $cart_item, $cart_item_key );
						?>
					</td>
					<td class="product-remove hidden-xs" data-title="Hủy bỏ">
						<?php
					...
					<td class=" visible-xs" >
						<?php
							echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
								'<a href="%s" class="remove" title="%s" data-product_id="%s" data-product_sku="%s">Hủy bỏ</a>',
								esc_url( WC()->cart->get_remove_url( $cart_item_key ) ),
								__( 'Remove this item', 'woocommerce' ),
								esc_attr( $product_id ),
								esc_attr( $_product->get_sku() )
